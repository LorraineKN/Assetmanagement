...
class AssetTransferInline(admin.TabularInline):
    model = AssetTransfer
    extra = 0
    fields = ('from_location', 'to_location', 'from_custodian', 'to_custodian', 'transfer_date')
    fk_name = 'asset'
...
class AssetAdmin(BaseAdmin):
    list_display = ('asset_id', 'name', 'status', 'category', 'location', 'custodian', 
                    'purchase_date', 'current_value', 'last_audited')
    list_filter = ('status', 'category', 'location', 'custodian', 'purchase_date', 'last_audited', *BaseAdmin.list_filter)
    ...
    fieldsets = (
        ('Basic Information', {
            'fields': ('asset_id', 'name', 'description', 'status', 'category', 'tags', 'image')
        }),
        ('Technical Details', {
            'fields': ('serial_number', 'model_number', 'manufacturer')
        }),
        ('Financial Information', {
            'fields': ('purchase_price', 'current_value', 'purchase_date', 'purchase_order', 
                      'warranty_expiry_date', 'expected_life_years', 'depreciation_rate')
        }),
    ...
    def get_queryset(self, request):
        ...
        return super().get_queryset(request).select_related(
            'category', 'vendor', 'location', 'custodian'
        ).prefetch_related('tags')


@admin.register(MaintenanceRecord)
class MaintenanceRecordAdmin(BaseAdmin):
    list_display = ('asset', 'maintenance_type', 'status', 'schedule_date', 'completion_date', 'cost')
    list_filter = ('maintenance_type', 'status', 'schedule_date', 'completion_date', *BaseAdmin.list_filter)
...
class AssetTransferAdmin(BaseAdmin):
    list_display = ('asset', 'from_location', 'to_location', 'from_custodian', 'to_custodian', 'transfer_date')
    list_filter = ('transfer_date', 'from_location', 'to_location', *BaseAdmin.list_filter)
    search_fields = ('asset__name', 'asset__asset_id', 'reason')
    autocomplete_fields = ['asset', 'from_location', 'to_location', 'from_custodian', 'to_custodian', 'approved_by']


@admin.register(DepreciationRecord)
class DepreciationRecordAdmin(BaseAdmin):
    list_display = ('asset', 'record_date', 'value_before', 'value_after', 'depreciation_amount')
...
class FinancialTransactionAdmin(BaseAdmin):
    list_display = ('transaction_type', 'amount', 'date', 'asset', 'vendor', 'reference_number')
    list_filter = ('transaction_type', 'date', 'vendor', *BaseAdmin.list_filter)
    search_fields = ('description', 'reference_number', 'asset__name', 'asset__asset_id')
...
class AssetManagementAdminSite(admin.AdminSite):
    ...
    def get_app_list(self, request):
        ...
        try:
            ...
            total_value = Asset.objects.aggregate(Sum('current_value'))['current_value__sum'] or 0
            pending_maintenance = MaintenanceRecord.objects.filter(status__in=['scheduled', 'in_progress']).count()
            
            # Add stats to the app_list
            for app in app_list:
                if app.get('app_label') == 'your_app_name':  # Replace with your actual app name
                    app['stats'] = {
